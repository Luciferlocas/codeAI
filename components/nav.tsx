...
const navItems = [
  { icon: Home, label: "Repositories", route: "/u/repository" },
  { icon: Code2, label: "AI Code Review", route: "/u/ai-code-review" },
  { icon: Cloud, label: "Cloud Security", route: "/u/cloud-security" },
  { icon: FileText, label: "How to Use", route: "/u/how-to-use" },
  { icon: Settings, label: "Settings", route: "/u/settings" },
  { icon: Phone, label: "Support", route: "/u/support" },
];

const owners = ["UtkarshDhairyaPanwar", "JohnDoe", "JaneSmith", "AlexJohnson"];
...
export default function Nav() {
  ...
  const handleNavClick = (label: string, route: string) => {
    ...
    if (route === "/") {
      ...
      router.push(`${route}?message=Coming Soon`);
    }
  };

  return (
    <>
      <aside className="w-64 flex-shrink-0 hidden md:flex flex-col gap-4 border-r p-6 bg-white">
